  ...
  return (
    <div className="bg-white/50 dark:bg-black/20 border border-black/5 dark:border-white/10 rounded-2xl p-4 mb-3 backdrop-blur-md shadow-sm relative flex flex-col gap-3 w-full">
      <div className="w-full flex justify-center items-center relative">
        <h3 className="font-bold text-blue-600 dark:text-blue-400 text-base text-center">LinkedIn Sales Page Calculation</h3>
      ...
      <div className="flex justify-center"><div className="flex items-center gap-2 text-xs font-medium text-gray-700 dark:text-gray-300 bg-blue-500/10 dark:bg-blue-500/20 px-4 py-1.5 rounded-full border border-blue-500/20"><Compass size={12} className="text-blue-500" /><span>Default Leads on SalesNav Page ➜</span><span className="font-bold text-blue-700 dark:text-blue-300">25</span></div></div>
      <div className="flex flex-nowrap items-center justify-center w-full mt-1 gap-6">
        <div className="flex items-center gap-3 w-[290px] justify-end"><input type="number" placeholder="Total Sales Results" value={totalSales} onChange={(e) => setTotalSales(e.target.value === '' ? '' : Number(e.target.value))} className="w-48 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg py-1.5 px-3 text-center text-xs outline-none text-gray-800 dark:text-white placeholder:text-gray-400 no-spinner focus:border-blue-400 transition-colors" /><div className="flex items-center gap-1 whitespace-nowrap text-xs text-gray-500 dark:text-gray-400"><span className="w-20 text-right">Approx. Page:</span><span className="font-bold text-sm text-gray-800 dark:text-white w-8 text-left tabular-nums">{approx}</span></div></div>
        <div className="w-px h-8 bg-gray-300 dark:bg-white/10"></div>
        <div className="flex items-center gap-3 w-[290px] justify-start"><input type="number" placeholder="Current Sales Page" value={currentPage} onChange={(e) => setCurrentPage(e.target.value === '' ? '' : Number(e.target.value))} className="w-48 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg py-1.5 px-3 text-center text-xs outline-none text-gray-800 dark:text-white placeholder:text-gray-400 no-spinner focus:border-blue-400 transition-colors" /><div className="flex items-center gap-1 whitespace-nowrap text-xs text-gray-500 dark:text-gray-400"><span className="w-20 text-right">Remain Page:</span><span className="font-bold text-sm text-gray-800 dark:text-white w-8 text-left tabular-nums">{remain}</span></div></div>
      </div>
    </div>
  );
};
...
const SentLinksSection = ({ total, setTotal, triggerConfirm, setLogs, onOpenLogs }: any) => {
  ...
  return (
    <div className="bg-white/50 dark:bg-black/20 border border-black/5 dark:border-white/10 rounded-2xl p-4 mb-3 backdrop-blur-md shadow-sm flex items-center justify-between w-full gap-3">
      <div className="flex items-center gap-2 w-[180px]"><input type="number" placeholder="Enter" value={val} onChange={(e) => setVal(e.target.value === '' ? '' : Number(e.target.value))} onKeyDown={(e) => e.key === 'Enter' && handleAdd()} className="w-32 bg-white dark:bg-white/5 border border-blue-500/30 rounded-lg py-2 px-3 text-center font-bold text-gray-800 dark:text-white outline-none placeholder:font-normal placeholder:text-gray-400 no-spinner text-sm focus:border-blue-500 transition-colors" /><button onClick={handleAdd} className="bg-blue-100 hover:bg-blue-200 dark:bg-blue-500/20 dark:hover:bg-blue-500/40 text-blue-600 dark:text-blue-300 border border-blue-500/30 p-2 rounded-lg transition-colors"><Check size={18} strokeWidth={3} /></button></div>
      <div className="w-px h-8 bg-gray-300 dark:bg-white/10"></div>
      <div className="flex items-center gap-2 flex-1 justify-center"><span className="text-blue-600 dark:text-blue-400 font-bold text-sm whitespace-nowrap w-[120px] text-right">Total Sent Links =</span><div className="bg-blue-100/50 dark:bg-blue-500/10 border border-blue-500/30 rounded-lg px-8 py-1 min-w-[100px] w-[100px] text-center flex justify-center items-center h-[34px]"><span className="text-lg font-bold text-blue-700 dark:text-blue-300 tabular-nums leading-none">{total}</span></div></div>
      <div className="w-px h-8 bg-gray-300 dark:bg-white/10"></div>
      <div className="flex items-center gap-2 w-[160px] justify-end"><button onClick={onOpenLogs} className="flex items-center gap-1 bg-white dark:bg-white/5 border border-blue-500/30 hover:bg-blue-50 dark:hover:bg-blue-500/20 text-blue-600 dark:text-blue-400 px-4 py-2 rounded-lg font-bold text-xs transition-all"><History size={14} /> Logs</button><button onClick={() => triggerConfirm("Reset total sent links and logs?", () => { setTotal(0); setLogs([]); })} className="flex items-center gap-1 bg-red-50 dark:bg-red-500/10 border border-red-500/30 hover:bg-red-100 dark:hover:bg-red-500/30 text-red-600 dark:text-red-400 px-4 py-2 rounded-lg font-bold text-xs transition-all"><RotateCcw size={14} /> Reset!</button></div>
    </div>
  );
};
        ...
        <div className="bg-white/50 dark:bg-black/20 border border-black/5 dark:border-white/10 rounded-2xl p-5 backdrop-blur-md shadow-sm h-full flex flex-col justify-between w-full">
            ...
            {currentEmails.length === 0 ? <div className="flex items-center justify-center h-full text-sm opacity-40 italic text-gray-500 dark:text-gray-400">Page {PAGES[pageIndex]} is empty</div> : 
                ...
                <div key={email.id} className="group flex items-center gap-3 p-2.5 rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 shadow-sm transition-all hover:shadow-md">
                    ...
                    {editingId === email.id ? <div className="flex-1 flex gap-2"><input value={editValue} onChange={(e) => setEditValue(e.target.value)} className="flex-1 bg-gray-50 dark:bg-black border border-blue-500 rounded px-2 py-1 text-sm outline-none text-gray-900 dark:text-white" autoFocus /><button onClick={saveEdit} className="text-green-500"><Check size={16} /></button></div> : <span onClick={() => handleCopy(email.id, email.text)} className={`flex-1 text-sm cursor-pointer select-none text-center font-medium transition-all ${email.copied ? 'text-blue-600 dark:text-blue-400 font-bold' : 'text-gray-700 dark:text-gray-300'}`}>{email.text}</span>}
      ...
      <div className="bg-blue-100/50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-500/20 rounded-2xl p-4 flex items-center justify-between">
         ...
         <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-blue-700 dark:text-blue-300 font-bold">
               ...
               <span className="bg-white dark:bg-black/20 px-8 py-1 rounded-lg border border-blue-300 min-w-[140px] text-center tabular-nums inline-block text-blue-700 dark:text-blue-300">{officeTotal.toLocaleString()}</span>
            </div>

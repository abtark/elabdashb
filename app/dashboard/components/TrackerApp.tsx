...
const StopwatchDisplay = ({ label, time, elapsed, isRunning, laps, onToggle, onReset, onLap, onDeleteLap, formatTime }: any) => (
  <div className="flex flex-col items-center w-full h-full gap-6">
    <div className="flex flex-col items-center justify-center bg-white/40 dark:bg-black/20 border border-black/5 dark:border-white/10 rounded-2xl p-8 backdrop-blur-md shadow-sm w-full max-w-2xl h-[400px] shrink-0 transition-all">
      <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-white flex items-center gap-3"><div className="w-8 h-8 flex items-center justify-center"><Clock size={28} className="text-orange-500" /></div>{label}<span className="text-lg font-mono text-black dark:text-white ml-2 font-bold opacity-80">{getDecimalHours(elapsed)}</span></h2>
  ...
  return (
    <div className="h-full flex flex-col relative w-full font-ubuntu gap-4">
      <div className="flex justify-center items-center gap-4 mb-0 shrink-0 relative min-h-[40px]">
        ...
        {/* UPDATED: Orange Color for NewTask Submenu */}

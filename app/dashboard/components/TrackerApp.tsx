...
// ... Interfaces (StopwatchState, TrackerAppProps) ...
...
const StopwatchDisplay = ({ 
  ...
  <div className="flex flex-col items-center w-full h-full gap-6">
    <div className="flex flex-col items-center justify-center bg-white/40 dark:bg-black/20 border border-black/5 dark:border-white/10 rounded-2xl p-8 backdrop-blur-md shadow-sm w-full max-w-2xl h-[400px] shrink-0 transition-all">
      <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-white flex items-center gap-3">
        ...
        {label}
        <span className="text-lg font-mono text-black dark:text-white ml-2 font-bold opacity-80">{getDecimalHours(elapsed)}</span>
      </h2>
      <div className="text-7xl sm:text-8xl font-mono font-bold text-gray-900 dark:text-white mb-8 tracking-wider tabular-nums w-full text-center select-none drop-shadow-sm">{time}</div>
      <div className="flex gap-6">
        <button onClick={onToggle} className={`p-6 rounded-full transition-all shadow-xl hover:scale-105 active:scale-95 border border-black/5 dark:border-white/10 ${isRunning ? 'bg-orange-500 text-white hover:bg-orange-600' : 'bg-green-500 text-white hover:bg-green-600'}`}>{isRunning ? <Pause size={40} fill="currentColor" /> : <Play size={40} fill="currentColor" className="ml-1" />}</button>
        <button onClick={isRunning ? onLap : onReset} className={`p-6 rounded-full transition-all shadow-xl hover:scale-105 active:scale-95 border border-black/5 dark:border-white/10 ${isRunning ? 'bg-blue-500 text-white hover:bg-blue-600' : 'bg-red-500 text-white hover:bg-red-600'}`}>{isRunning ? <Flag size={40} /> : <RotateCcw size={40} />}</button>
      </div>
    </div>
    {laps.length > 0 && (
        <div className="w-full max-w-2xl h-[200px] shrink-0">
            <div className="w-full h-full bg-white/40 dark:bg-black/20 border border-black/5 dark:border-white/10 rounded-2xl p-4 backdrop-blur-md shadow-sm overflow-hidden flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-300">
                <h3 className="text-sm font-bold text-gray-600 dark:text-gray-300 mb-2 uppercase tracking-wider text-center sticky top-0 bg-transparent z-10">Laps</h3>
                <div className="overflow-y-auto custom-scrollbar pr-2 flex-1">
                    {laps.map((lapTime: number, index: number) => (
                        <div key={index} className="flex justify-between items-center py-2.5 border-b border-gray-300 dark:border-white/10 last:border-0 text-sm font-mono font-medium text-gray-800 dark:text-gray-200 hover:bg-white/20 dark:hover:bg-white/10 rounded px-2 transition-colors group">
                            <span className="opacity-70 w-16 text-gray-700 dark:text-gray-400">Lap {laps.length - index}</span>
                            <span className="flex-1 text-center cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 active:scale-95 transition-all select-none font-bold" onClick={() => navigator.clipboard.writeText(formatTime(lapTime))} title="Click to copy">{formatTime(lapTime)}</span>
                            <button onClick={() => onDeleteLap(index)} className="w-8 h-8 flex items-center justify-center text-red-500 hover:text-white hover:bg-red-500 rounded-full transition-all"><X size={16} /></button>
                        </div>
                    ))}
                </div>
            </div>

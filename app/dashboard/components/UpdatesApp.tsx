...
export default function UpdatesApp({ onClose, totalSentLinks, entriesCounts, mainHourDecimal, ntHourDecimal, onGlobalReset, resetSignal }: UpdatesAppProps) {
  ...
  const [otCopied, setOtCopied] = useState(false);
  ...
  const generateSummary = () => { const parts = []; Object.entries(entriesCounts).forEach(([key, val]) => { if (val > 0) parts.push(`${LABELS[key]}: ${val}`); }); if (ntHourDecimal > 0) { const ntStr = `NewTask ${ntHourDecimal.toFixed(2)}h` + (totalSentLinks > 0 ? ` (${totalSentLinks})` : ''); parts.push(ntStr); } else if (totalSentLinks > 0) { parts.push(`NewTask (${totalSentLinks})`); } const activeToggles = toggles.filter(t => t.checked).map(t => t.label); if (activeToggles.length > 0) parts.push(`[${activeToggles.join(', ')}]`); return parts.join(', '); };
  const summaryText = generateSummary(); const displayText = summaryText || "No updates available."; const isCopyable = summaryText.length > 0; const totalEntries = Object.values(entriesCounts).reduce((a,b)=>a+b,0);
  const handleCopy = () => { if (!isCopyable) return; navigator.clipboard.writeText(summaryText); setCopied(true); setTimeout(() => setCopied(false), 1500); };
  ...
  return (
    <div className="h-full flex flex-col relative gap-3 w-full px-2 font-ubuntu">
      ...
      <div className="bg-white/40 dark:bg-black/20 border border-white/20 dark:border-white/10 rounded-2xl p-3 flex justify-between items-center text-xs sm:text-sm font-bold shadow-sm backdrop-blur-md shrink-0">
         ...
         <div className="h-4 w-px bg-gray-300 dark:bg-white/10"></div>
         <button onClick={() => setResetConfirmOpen(true)} className="flex items-center gap-1 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 px-2 py-1 rounded-lg transition-colors border border-red-500/20"><RotateCcw size={12}/> Reset</button>
      </div>

      <div className="flex gap-3 w-full shrink-0">
         <div className="flex-1 bg-white/40 dark:bg-black/20 border border-white/20 dark:border-white/10 rounded-2xl p-3 text-center shadow-sm"><div className="text-[10px] font-bold mb-1 flex items-center justify-center gap-1 text-gray-500 dark:text-gray-400 uppercase tracking-wider"><Hourglass size={10}/> Main Hour</div><div className="text-2xl font-bold text-gray-800 dark:text-white tabular-nums">{mainHourDecimal.toFixed(2)}</div></div>
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-2 flex-1 overflow-y-auto custom-scrollbar p-1 content-start">
         {toggles.map(item => (
            <div key={item.id} className={`group flex justify-between items-center px-3 rounded-xl border transition-all shadow-sm h-9 ${item.checked ? 'bg-green-100/80 border-green-200 dark:bg-green-900/40 dark:border-green-500/30' : 'bg-white/40 border-transparent dark:bg-white/5 hover:border-white/30'}`}>
               <div className="flex items-center gap-2 flex-1 min-w-0">
                   <button onClick={() => handleEditToggle(item.id)} className="text-gray-400 hover:text-blue-500 opacity-0 group-hover:opacity-100 transition-opacity">{item.editing ? <Check size={12} className="text-green-500"/> : <Edit2 size={12}/>}</button>
                   {item.editing ? <input value={item.label} onChange={(e) => handleLabelChange(item.id, e.target.value)} className="bg-transparent border-b border-blue-500 outline-none w-full text-xs font-medium text-gray-800 dark:text-white" autoFocus onKeyDown={e => e.key === 'Enter' && handleEditToggle(item.id)} /> : <span className={`text-xs font-medium truncate select-none ${item.checked ? 'text-green-800 dark:text-green-200 font-bold' : 'text-gray-700 dark:text-gray-200'}`} title={item.label}>{item.label}</span>}
               </div>
               <div onClick={() => handleToggleCheck(item.id)} className={`cursor-pointer w-8 h-4 rounded-full p-0.5 flex items-center shadow-inner transition-colors duration-300 ${item.checked ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}`}><motion.div initial={false} animate={{ x: item.checked ? 16 : 0 }} transition={{ type: "spring", stiffness: 700, damping: 30 }} className="w-3 h-3 bg-white rounded-full shadow-sm" /></div>
            </div>
         ))}
      </div>

      <div className="relative shrink-0">
        <div onClick={handleCopy} className={`bg-white/60 dark:bg-black/40 border border-white/30 dark:border-white/10 rounded-2xl p-4 text-center transition-all duration-300 shadow-sm min-h-[80px] flex flex-col justify-center items-center ${isCopyable ? 'cursor-pointer hover:bg-white/80 dark:hover:bg-white/10' : 'cursor-not-allowed opacity-70'} ${copied ? 'bg-green-100/80 dark:bg-green-900/30 border-green-500/50 scale-[1.02]' : ''}`}><div className="font-medium text-sm text-gray-800 dark:text-gray-200 break-words leading-relaxed select-none w-full">{displayText}</div></div>
        <AnimatePresence>{copied && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute bottom-2 left-0 right-0 text-center pointer-events-none"><span className="text-[10px] uppercase font-bold text-green-600 dark:text-green-400 bg-white/80 dark:bg-black/80 px-2 py-0.5 rounded-full shadow-sm">Copied to clipboard!</span></motion.div>)}</AnimatePresence>
      </div>

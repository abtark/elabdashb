...
export default function SnacksApp({ onClose }: { onClose: () => void }) {
  ...
  const sendToDiscord = async () => { if (!selectedSlot) return; setIsSending(true); setIsSent(false); const row = snackRows[selectedSlot.dayIndex]; const snackName = selectedSlot.type === 'morning' ? row.morning : row.evening; const timeLabel = selectedSlot.type === 'morning' ? 'Morning Snacks' : 'Evening Snacks'; const content = `**${timeLabel}: ${snackName}**`; const webhookURL = "https://discordapp.com/api/webhooks/1414906899404558386/8Orq3LhblO-8_CTk3Bt_1o2ZjiY7kaTreuKIc8qSpj_tbHPoLzU8SkU1HdkBN-nPu8gd"; try { await fetch(webhookURL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ content }) }); setIsSending(false); setIsSent(true); setTimeout(() => { setIsSent(false); setSelectedSlot(null); }, 2000); } catch (e) { alert("Failed to send."); setIsSending(false); console.error(e); } };

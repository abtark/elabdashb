...
export default function EntriesApp({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="h-full flex flex-col relative gap-4 max-w-[95%] mx-auto">
      ...
      {/* 1. Sub-Menu / Top Bar */}
      <div className="flex justify-between items-center mb-0 shrink-0 select-none">
        
        {/* Left: Total & Logs */}
        <div className="flex items-center gap-3">
          <div className="bg-white/20 dark:bg-white/5 border border-white/20 dark:border-white/10 rounded-full px-4 py-1.5 flex items-center gap-2 text-sm font-bold text-gray-700 dark:text-gray-300">
             <span>Today's Total:</span>
             <span className="text-blue-600 dark:text-blue-400 text-lg">{getTotal()}</span>
          </div>
          <button onClick={() => setShowLogs(true)} className="flex items-center gap-1.5 bg-white/20 dark:bg-white/5 border border-blue-500/30 hover:bg-blue-50 dark:hover:bg-blue-500/20 text-blue-600 dark:text-blue-400 px-4 py-1.5 rounded-full font-bold text-xs transition-all">
             <History size={14} /> Logs
           </button>
           <button onClick={() => setResetConfirmOpen(true)} className="flex items-center gap-1.5 bg-red-50/50 dark:bg-red-500/10 border border-red-500/30 hover:bg-red-100 dark:hover:bg-red-500/30 text-red-600 dark:text-red-400 px-4 py-1.5 rounded-full font-bold text-xs transition-all">
        ...
        <button 
          onClick={onClose} 
          className="flex items-center gap-1.5 bg-red-500 hover:bg-red-600 text-white px-4 py-1.5 rounded-full font-bold text-xs transition-all shadow-md"
        >
          Close <X size={14} />
        </button>
      </div>
      ...
      <div className="bg-white/40 dark:bg-black/20 border border-white/20 dark:border-white/10 rounded-2xl p-2 shadow-sm backdrop-blur-md overflow-x-auto custom-scrollbar">
        <div className="flex items-center justify-between min-w-[700px] gap-2">
          {CATEGORIES.map((cat) => (
            <div key={cat.key} className="flex flex-col items-center justify-center flex-1 p-2 bg-blue-50/50 dark:bg-white/5 rounded-xl border border-blue-100 dark:border-white/5">
          ...
          {CATEGORIES.map((cat) => (
            <div key={cat.key} className="flex items-center gap-2 group">
              ...
              <button 
                ...
                className="p-2 text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-lg transition-colors opacity-50 group-hover:opacity-100"
                title="Edit Label"
              >
                <Edit3 size={16} />
              </button>

    ...
    <div className="h-full flex flex-col relative gap-4 w-full px-2 font-ubuntu">
      ...
      <div className="flex justify-between items-center bg-white/40 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-2xl p-4 shadow-sm backdrop-blur-md shrink-0">
         <div className="flex items-center gap-3">
            ...
            {/* UPDATED: Green background and text */}
            <div className="h-10 flex items-center justify-center bg-green-100/50 dark:bg-green-500/10 border border-green-500/30 text-[#22C55E] dark:text-green-400 px-10 rounded-lg font-bold text-lg min-w-[80px]">
                {getTotal()}
            </div>
         </div>
         <div className="flex gap-3">
            {/* UPDATED: Logs Button Green */}
            <button onClick={() => setShowLogs(true)} className="h-10 flex items-center gap-1 bg-white dark:bg-white/5 border border-green-500/30 text-green-600 hover:bg-green-50 dark:hover:bg-green-500/20 px-5 rounded-lg text-xs font-bold transition-all shadow-sm">
      ...
      <div className="bg-white/40 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-2xl p-3 shadow-sm backdrop-blur-md shrink-0">
         <div className="grid grid-cols-3 gap-3 mb-3">
            {['cat1','cat2','cat3'].map(k => (
               // UPDATED: Very Light Green Background for Cards
               <div key={k} className="flex flex-col items-center p-3 bg-green-50/50 dark:bg-white/5 rounded-xl border border-green-100 dark:border-white/5 shadow-sm">
                  <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{labels[k as CategoryKey]}</span>
                  {/* UPDATED: Green Number */}
                  <span className="text-xl font-bold text-[#22C55E] dark:text-green-400">{counts[k as CategoryKey]}</span>
               </div>
            ))}
         </div>
